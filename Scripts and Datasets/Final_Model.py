...
# Train: 2000–2017 | Test: 2018–2019 (per country)
# Base Models:
...
#
# Stacking:
#   - RMSE-based weighted average of:
#       Ensemble 1: RFR, XGB, ADA, SVR, LSTM
#       Ensemble 2: RFR, XGB, ADA, LSTM, TCN
#
# Output:
#   - stack_results_final_2019/stack_train_2000_2017_base_preds.csv
#   - stack_results_final_2019/stack_test_2018_2019_base_preds.csv
# ============================================================
...
# Config
# ============================================================
DATA_PATH_IMPUTED = "Final_Dataset_Imputed.csv"
RESULTS_DIR = "stack_results_final_2019"
...
# ============================================================
# 4b) TCN PIPELINE (MULTIVARIATE + EMBEDDINGS)
# ============================================================
...
# ============================================================
# 5b) Compute aligned LSTM & TCN metrics on 2018–2019 test
# ============================================================
...
# 9) SAVE
# ============================================================
train_stack_path = os.path.join(RESULTS_DIR, "stack_train_2000_2017_base_preds.csv")
test_stack_path  = os.path.join(RESULTS_DIR, "stack_test_2018_2019_base_preds.csv")
...
# ============================================================
#  SHAP ANALYSIS FOR XGB (GLOBAL IMPORTANCE + PLOTS)
# ============================================================
import shap
...
# -----------------------------
# 2. Fit SHAP TreeExplainer
# -----------------------------
expl_xgb = shap.TreeExplainer(base_models_tree["XGB"])

shap_vals = expl_xgb.shap_values(X_test_base)

# -----------------------------
# 3. Compute GLOBAL SHAP importance
# -----------------------------
shap_abs_mean = np.abs(shap_vals).mean(axis=0)  # average absolute shap value per feature
shap_norm = shap_abs_mean / shap_abs_mean.sum() # normalize to sum = 1

shap_global_df = pd.DataFrame({
    "Feature": numeric_features,
    "MeanAbsSHAP": shap_abs_mean,
    "NormalizedImportance": shap_norm
}).sort_values("NormalizedImportance", ascending=False)

print("\n=== Global SHAP Importance (Normalized) ===")
print(shap_global_df)

# -----------------------------
# 4. Plot: Normalized SHAP Global Bar Plot
# -----------------------------
plt.figure(figsize=(10, 6))
sns.barplot(
    data=shap_global_df,
    x="NormalizedImportance",
    y="Feature",
    hue="Feature",
    palette="viridis",
    legend=False
)
plt.title("Global SHAP Feature Importance (XGBoost, Normalized)")
...
# -----------------------------
# 5. Plot: SHAP Beeswarm
# -----------------------------
shap.summary_plot(shap_vals, X_test_base, feature_names=numeric_features)
